...
oneStage <- function(N = 350, N2 = 1000, iter = 20, rho = 0.2, No.Var.H = 20, No.Var.Hsub = 7, Method = "REST-L", prop.model = "correct"){
  
  select1<-rep(NA,iter) # percent consistent with optimal regime
  ...
  for(i in 1:iter){
    ...
    # generate true optimal for regime at stage 1
    g1.opt<-(x1>-1)*((x2>-0.5)+(x2>0.5))
  ...
  return(c(No.Stages = 1, No.RXperStage = 3, rho=rho, No.Var.H=No.Var.H, No.Var.Hsub=No.Var.Hsub, n.Sample = N, n.TestSet = N2, propensity.model = prop.model, true.DTR = "tree", B = iter, Method = Method,
