...
########################################################
# simulation - two stages - Section 4.3
########################################################

twoStage <- function(N = 350, N2 = 1000, iter = 20, rho = 0.2, No.Var.H = 20, No.Var.Hsub = 7, Method = "REST-L", prop.model = "correct"){

select1<-select2<-selects<-rep(NA,iter) 
EYs<-rep(NA,iter) 
...
############### stage 1 data simulation ##############
    ...
    if(prop.model == "correct"){
      pis2.hat<-M.propen(A2,cbind(R1,X0[,id2]))
    } else if(prop.model == "incorrect"){
    ...
    # stage 2 outcome R2
    R2<-exp(1.18+0.2*X0[,id2]-abs(1.5*x3+2)*(A2-g2.opt)^2)+rnorm(N,0,1)

########### Using Backward induction #############################
############### stage 2 Estimation ###############################

if(Method == "REST-L"){
  REG2<-Reg.mu.modified.Hsub(Y=R2,As=cbind(A1,A2),Hsub=cbind(X0[,1:No.Var.Hsub],R1), HsubC=cbind(X0[,(No.Var.Hsub+1):No.Var.H]))
  mus2.reg<-REG2$mus.reg
  
  tree2<-DTRtree.modified.HSUB(R2,A2,Hsub=cbind(X0[,1:No.Var.Hsub],A1,R1),HsubC=cbind(X0[,(No.Var.Hsub+1):No.Var.H]),pis.hat=pis2.hat,mus.reg=mus2.reg,lambda.pct=0.05,minsplit=max(0.05*N,20))
...
############################################
REG1<-Reg.mu.modified.Hsub(Y=PO.tree,As=A1,Hsub=cbind(X0[,1:No.Var.Hsub]), HsubC=cbind(X0[,(No.Var.Hsub+1):No.Var.H])) #
mus1.reg<-REG1$mus.reg

tree1<-DTRtree.modified.HSUB(PO.tree,A1,Hsub=cbind(X0[,1:No.Var.Hsub]),HsubC=cbind(X0[,(No.Var.Hsub+1):No.Var.H]),pis.hat=pis1.hat,mus.reg=mus1.reg, lambda.pct=0.05,minsplit=max(0.05*N,20)) ## 
} else if(Method == "T-RL"){ 
  ...
  tree1<-DTRtree.modified(PO.tree,A1,H=X0,pis.hat=pis1.hat,lambda.pct=0.05,minsplit=max(0.05*N,20))
    ...
    QLinear1 <- lm(R1.tilde ~ A1*., data = data3[,c(1:(No.Var.H+1),No.Var.H+9)]) 
  
} else if(Method == "Q-Linear-REST"){
  
  if(No.Var.Hsub == 7 & No.Var.H == 20){
    formula <- R2 ~ A2*(x1+x2+x3+x4+x5+x6+x7+A1+R1) + x8+x9+x10+x11+x12+x13+x14+x15+x16+x17+x18+x19+x20
  } else if(No.Var.Hsub == 10 & No.Var.H == 50){
    formula <- R2 ~ A2*(x1+x2+x3+x4+x5+x6+x7+x8+x9+x10+A1+R1) + x11+x12+x13+x14+x15+x16+x17+x18+x19+x20+x21+x22+x23+x24+x25+x26+x27+x28+x29+x30+x31+x32+x33+x34+x35+x36+x37+x38+x39+x40+x41+x42+x43+x44+x45+x46+x47+x48+x49+x50
    ...
    formula <- R2 ~ A2*(x1+x2+x3+x4+x5+x6+x7+x8+x9+x10+x11+x12+x13+x14+x15+x16+x17+x18+x19+x20+x21+x22+x23+x24+x25+x26+x27+x28+x29+x30+x31+x32+x33+x34+x35+A1+R1)+x36+x37+x38+x39+x40+x41+x42+x43+x44+x45+x46+x47+x48+x49+x50
  } 
  QLinear2 <- lm(formula)
  ...
  if(No.Var.Hsub == 7 & No.Var.H == 20){
    formula2 <- R1.tilde ~ A1*(x1+x2+x3+x4+x5+x6+x7) + x8+x9+x10+x11+x12+x13+x14+x15+x16+x17+x18+x19+x20
  } else if(No.Var.Hsub == 10 & No.Var.H == 50){
  ...
  QLinear1 <- lm(formula2, data = data3[,c(1:(No.Var.H+1),No.Var.H+9)]) 
  ...
  QRF1 <- randomForest(R1.tilde ~ ., data = data3[,c(1:(No.Var.H+1),No.Var.H+9)], na.action = na.omit) 
  ...
  QRF1 <- randomForest(formula4, data = data3[,c(1:(No.Var.H+1),No.Var.H+9)], na.action = na.omit)
...
# true optimal for regime at stage 1
  ...
  R2<-exp(1.18+0.2*X0[,id2])+rnorm(N2,0,1)
...
####### stage 2 prediction #######
  ...
  g2.tree<-predict.DTR.modified2(tree2,newdata=data.frame(X0[,c(1:No.Var.Hsub)],A1=g1.tree,R1=R1.tree)) 
...
# predict R2
  R2.tree<-exp(1.18+0.2*X0[,id2]-abs(1.5*x3+2)*(g2.tree-g2.opt.tree)^2)+rnorm(N2,0,1)
...
return(c(No.Stages = 2, No.RXperStage = 3, rho=rho, No.Var.H=No.Var.H, No.Var.Hsub=No.Var.Hsub, n.Sample = N, n.TestSet = N2, propensity.model = prop.model, true.DTR = "tree", B = iter, Method = Method,
