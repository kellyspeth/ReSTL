...
twoStage2 <- function(N = 350, N2 = 1000, iter = 20, rho = 0.2, No.Var.H = 20, No.Var.Hsub = 7, Method = "REST-L", prop.model = "correct"){
...
for(i in 1:iter){
  set.seed(i)
  CorrMat <- Create.Cor.Mat5(No.Var.H, No.Var.Hsub, rho) 
  Z <- rbinom(N, 1, 0.4)
...
####### stage 2 prediction #######
  ...
  g2.opt.tree<-(R1.tree>0.5)*(x3>-1)+(x3>0) ## 
