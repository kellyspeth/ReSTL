...
twoStage_nontree <- function(N = 350, N2 = 1000, iter = 20, rho = 0.2, No.Var.H = 20, No.Var.Hsub = 7, Method = "REST-L", prop.model = "correct"){
...
# optimal g2.opt
g2.opt <- (abs(x3)> 0.60)*(R1>0.4) + (R1^2>2.5)
# stage 2 outcome R2
...
############################################
set.seed(i+10000)
X <- matrix((MASS::mvrnorm(n=N2, mu=rep(0,No.Var.H), Sigma=CorrMat)),nrow=N2) 
if(No.Var.H == 20){
  x1<-X[,1]; x2<-X[,2]; x3<-X[,3]; x4<-X[,4]; x5<-X[,5]; x6<-X[,6]; x7<-X[,7]; x8<- X[,8]; x9<-X[,9]; x10<-X[,10]; 
  x11<-X[,11]; x12<-X[,12]; x13<-X[,13]; x14<-X[,14]; x15<-X[,15]; x16<-X[,16]; x17<-X[,17]; x18<-X[,18]; x19<-X[,19]; x20<-X[,20];
  X0<-cbind(x1,x2,x3,x4,x5,x6,x7,x8,x9,x10,x11,x12,x13,x14,x15,x16,x17,x18,x19,x20)
...
####### stage 2 prediction #######
  ...
  g2.opt.tree <- (abs(x3)> 0.60)*(R1.tree>0.4) + (R1.tree^2>2.5)
